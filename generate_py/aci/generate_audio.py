...
DATA=json.loads((ROOT/'content/aci.json').read_text())
BASE=ROOT/'public/audio/matematik/aci'
...
async def main():
    ...
    for i,item in enumerate(DATA['shorts'],1):
        ...
        reveal=question+150; end=reveal+answer
        timings['shorts'].append({'qEnd':question,'reveal':reveal,'aEnd':end,'congrats':end,'frames':end+45})
    out=ROOT/'src/aci/timings.json'; out.parent.mkdir(parents=True,exist_ok=True)
