...
DATA=json.loads((ROOT/'content/hucre.json').read_text())
BASE=ROOT/'public/audio/fen/hucre'
...
async def main():
    ...
    for i,item in enumerate(DATA['shorts'],1):
        ...
        q_end=max(1,question-32)
        reveal=q_end+150; end=reveal+answer
        timings['shorts'].append({'qEnd':q_end,'reveal':reveal,'aEnd':end,'congrats':end,'frames':end+45})
    out=ROOT/'src/hucre/timings.json'; out.write_text(json.dumps(timings,indent=2)+'\n')
