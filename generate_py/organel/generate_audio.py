...
DATA=json.loads((ROOT/'content/organel.json').read_text())
BASE=ROOT/'public/audio/fen/organel'
    ...
    if not path.exists() or path.stat().st_size<1000:
        for attempt in range(3):
            try:
                ...
                if attempt==2: raise
    seconds=duration(path); print(f'{path.relative_to(ROOT)}: {seconds:.3f}s',flush=True)
...
async def main():
    ...
    item=DATA['shorts'][0]; target=BASE/'shorts'/'1'
    ...
    (ROOT/'src/organel/timings.json').write_text(json.dumps(timings,indent=2)+'\n')
