...
DATA=json.loads((ROOT/'content/mirasimiz.json').read_text())
BASE=ROOT/'public/audio/sosyal/mirasimiz'
...
async def main():
    ...
    for group in ('main','kurz'):
        for i,scene in enumerate(DATA[group]):
            ...
            timings[group].append({'audioFrames':audio,'frames':audio+(0 if group=='kurz' and i==len(DATA[group])-1 else 12)})
    for i,item in enumerate(DATA['shorts'],1):
        ...
        q_end=question; reveal=q_end+150; end=reveal+answer
        timings['shorts'].append({'qEnd':q_end,'reveal':reveal,'aEnd':end,'congrats':end,'frames':end+45})
    (ROOT/'src/mirasimiz').mkdir(parents=True,exist_ok=True)
    (ROOT/'src/mirasimiz/timings.json').write_text(json.dumps(timings,indent=2)+'\n')
