...
- Keep each Kurz companion at or below 70% of its corresponding final main video's measured total duration. The Kurz video may be shorter than this limit but must never exceed it. Measure the completed main video first, set the Kurz maximum duration to `main duration × 0.70`, and build the shortened narration and scene plan within that budget. Derive every visual and scene timing from the generated Kurz audio so narration and animation remain synchronized, then verify the final measured ratio before delivery.
